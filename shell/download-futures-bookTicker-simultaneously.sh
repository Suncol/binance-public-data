# Bash script who permit to download the perpetuals futures bookTicker simultaneously.
...
OUTPUT_BASE_DIR="/home/nas/user_data/suncong/binance_data/bookTicker/${CM_OR_UM}"

# First we verify if the CM_OR_UM is correct, if not, we exit
if [ "$CM_OR_UM" = "cm" ] || [ "$CM_OR_UM" == "um" ]; then
  BASE_URL="https://data.binance.vision/data/futures/${CM_OR_UM}/monthly/bookTicker"
else
...
for symbol in ${SYMBOLS[@]}; do
    for year in ${YEARS[@]}; do
        for month in ${MONTHS[@]}; do
            url="${BASE_URL}/${symbol}/${symbol}-bookTicker-${year}-${month}.zip"
            output_dir="${OUTPUT_BASE_DIR}/${symbol}"
            download_url "${url}" "${output_dir}" &
        done
        wait
    done
    echo "Downloaded all files for ${symbol}"
done
