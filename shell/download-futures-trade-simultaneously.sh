# Bash script who permit to download the perpetuals futures trade simultaneously.
...
OUTPUT_BASE_DIR="/home/nas/user_data/suncong/binance_data/trades/${CM_OR_UM}"

# First we verify if the CM_OR_UM is correct, if not, we exit
if [ "$CM_OR_UM" = "cm" ] || [ "$CM_OR_UM" == "um" ]; then
  BASE_URL="https://data.binance.vision/data/futures/${CM_OR_UM}/monthly/trades"
else
...
for symbol in ${SYMBOLS[@]}; do
    for year in ${YEARS[@]}; do
        for month in ${MONTHS[@]}; do
            url="${BASE_URL}/${symbol}/${symbol}-trades-${year}-${month}.zip"
